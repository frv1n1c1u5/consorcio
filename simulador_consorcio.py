...
def fetch_index(series_id: str, start: str, end: str) -> pd.Series:
    """Busca índice mensal (fator = 1 + variação) no SGS do BCB."""
    url = (
...
with st.sidebar:
    ...
    juros_ano = st.number_input(
        ...
        min_value=0.0, value=12.0, step=0.1, format="%.2f"
    )
...
if idx_choice == "Fixo 5%":
    ...
    factors = [f12 ** ((m-1)//12) for m in range(1, prazo_cons+1)]
else:
    ...
    f12 = last12.prod()
    st.metric(f"Acumulado 12m ({idx_choice})", f"{(f12-1)*100:.2f}%")
    factors = [f12 ** ((m-1)//12) for m in range(1, prazo_cons+1)]
...
fig_cf = px.line(
    df_cf, x="Mês", y=["Financiamento","Consórcio"],
...
fig.add_trace(go.Bar(x=x, y=fin_pad, name="Financiamento", marker_color="#2081E2", width=0.6))
for m in flips:
    fig.add_vline(x=m, line_dash="dash", line_color="yellow",
                  annotation_text=f"Mês {m}", annotation_position="top right")
fig.update_layout(
    template="plotly_dark", barmode="overlay", bargap=0.15,
...
with st.expander("📌 Metodologia"):
    st.markdown("""
- **Fluxo de Caixa**: saldo acumulado da entrada e parcelas.  
- **VPL**: descontado à taxa informada.  
- **TIR**: anualizada a partir da série de CF.  
- **Reajuste**: fixo 5% ou real via IPCA, INPC, IGP-M (últimos 12m).  
